...
public interface OrderRepo extends JpaRepository<Order, Long>{
	...
	@Query(value = "SELECT o.order_id as orderId,p.category as category,p.product_name as productName,uc.quantity as quantity, "
			...
			+ "o.status as status FROM order_details o inner join cart_order co "
			+ "on o.order_id=co.order_id inner join user_cart_details uc "
			+ "on co.cart_id=uc.cart_id inner join product_details p on uc.product_id=p.product_id "
			+ "inner join user_details u on uc.user_id=u.retailer_id "
